...
class TestFusedMultiTransformerInt8Op(OpTest):
    ...
    def config(self):
        ...
        self.input_scale = 127.0 / 50.0
        self.output_scale = 50.0/(127.0 * 127.0)
    ...
    def GetBaselineOut(self):
        ...
        for i in range(self.layers):
            ...
            # quant ln1_out
            ln1_out = paddle.cast(paddle.cast(paddle.round(ln1_out * self.input_scale),'int8'), 'float64')
            print("ln1_out", ln1_out)

            q = paddle.matmul(ln1_out, self.q_weight_tensor)
            # de quant
            print("before dequant q", q)
            q = paddle.cast(q, self.x_type)  * self.output_scale
            q = q + self.q_proj_bias_tensor
            q = tensor.reshape(x=q, shape=[0, 0, self.num_heads, self.head_dim])
            q_out = tensor.transpose(x=q, perm=[0, 2, 1, 3])

            k = paddle.matmul(ln1_out, self.k_weight_tensor)
            k = paddle.cast(k, self.x_type) * self.output_scale
            k = k + self.k_proj_bias_tensor
            v = paddle.matmul(ln1_out, self.v_weight_tensor)
            v = paddle.cast(v, self.x_type) * self.output_scale
            ...
            print("fmha_out")
            [r0, r1, r2, r3] = fmha_out.shape
            for i in range(r2):
                for j in range(r3):
                    print(fmha_out.numpy()[0,0,i,j], end=' ')
                print('\n')
            print('\n')
            if self.debug:
                print('fmha out is')
            ...
            out_linear_in = paddle.cast(paddle.cast(paddle.round(out_linear_in * self.input_scale),'int8'), 'float64')
            print("fmha_out quantize: ")
            [r0, r1, r2] = out_linear_in.shape
            for j in range(r2):
                print(out_linear_in.numpy().astype('int32')[0,0,j], end=' ')
            print('\n')
            print('\n')
            print(self.out_weight_tensor)
            out = paddle.matmul(out_linear_in, self.out_weight_tensor)
            print("out linear out  ", out.numpy().astype('int32'))
            # print("true is", np.dot(out_linear_in.numpy().astype('int32'), self.out_weight_tensor.numpy().astype('int32')))
            out = paddle.cast(out, self.x_type) * self.output_scale
            ...
            print("ln out before quant", ffn_ln_out.numpy())
            ffn_ln_out = paddle.cast(paddle.cast(paddle.round(ffn_ln_out * self.input_scale),'int8'), 'float64')
            print("ln out ", ffn_ln_out.shape)
            [r0, r1, r2] = ffn_ln_out.shape
            for i in range(r1):
                for j in range(r2):
                    print(ffn_ln_out.numpy().astype('int32')[0, i, j], end=' ')
                print('\n')
            print('\n')
            # print("ffn1 weight ", self.ffn1_weight_tensor)
            ffn1_out = paddle.matmul(ffn_ln_out, self.ffn1_weight_tensor)
            print("ffn1 out ", ffn1_out.numpy().astype('int32'))
            # true_out = np.dot(ffn_ln_out.numpy(), self.ffn1_weight_tensor.numpy().astype('int8')).astype('int32')
            # print("true ffn1 out ", true_out)

            ffn1_out = paddle.cast(ffn1_out, self.x_type) * self.output_scale
            ffn1_out = ffn1_out + self.ffn1_proj_bias_tensor
            ffn1_out = self.dropout(self.activation(ffn1_out))

            ffn1_out = paddle.cast(paddle.cast(paddle.round(ffn1_out * self.input_scale),'int8'), 'float64')
            print("ffn quant:",ffn1_out)
            ffn2_out = paddle.matmul(ffn1_out, self.ffn2_weight_tensor)
            print("ffn2 out ", ffn2_out.numpy().astype('int32'))
            ffn2_out = paddle.cast(ffn2_out, self.x_type) * self.output_scale
            ffn2_out = ffn2_out + self.ffn2_proj_bias_tensor
            print("ffn2 bias out ", ffn2_out)
    ...
    def GetFusedMultiTransformerOut(self):
        ...
        qkv_bias = np.concatenate(
                ( self.q_proj_bias_tensor.numpy(),  self.k_proj_bias_tensor.numpy(),  self.v_proj_bias_tensor.numpy()))
        ...
        for i in range(self.layers):
            ...
            qkv_in_scale.append(self.input_scale)
            out_linear_in_scale.append(self.input_scale)
            ffn1_in_scale.append(self.input_scale)
            ffn2_in_scale.append(self.input_scale)
            if self.has_cache_kv:
                cache_kvs.append(paddle.to_tensor(cache_kv,
                                                  stop_gradient=True))

        final_out = fused_multi_transformer_int8(x,
                                            ln_scales,
                                            ln_biases,
                                            qkv_weights,
                                            qkv_biases,
                                            ...
                                            ring_id=-1,
                                            name=None,
                                            qkv_out_scales=paddle.ones([self.layers, 3*self.embed_dim], 'float32') * self.output_scale,
                                            out_linear_out_scales=paddle.ones([self.layers, self.embed_dim], 'float32') * self.output_scale,
                                            ffn1_out_scales=paddle.ones([self.layers, 4 * self.embed_dim], 'float32') * self.output_scale,
                                            ffn2_out_scales=paddle.ones([self.layers, self.embed_dim], 'float32') * self.output_scale,
...
class TestFusedMultiTransformerOpInt8CacheKV(TestFusedMultiTransformerInt8Op):
