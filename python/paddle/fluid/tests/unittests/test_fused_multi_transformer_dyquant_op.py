...
def fused_multi_transformer_dyquant(
    x,
    ln_scales,
    ln_biases,
    qkv_weights,
    ...
    cache_kv_out, final_out = _legacy_C_ops.fused_multi_transformer_dyquant(
        x,
        ln_scales,
        ln_biases,
        qkv_weights,
...
@unittest.skipIf(
    ...
    "FusedMultiTransformerDyquant requires CUDA >= 11.2 and CUDA_ARCH >= 8",
)
class TestFusedMultiTransformerDyquantOp(unittest.TestCase):
    def setUp(self):
        np.random.seed(0)
        self.config()
        ...
        self.__class__.op_type = "fused_multi_transformer_dyquant"
    ...
    def generate_input_data(self):
        ...
        self.qkv_in_scales = []
        self.qkv_out_scales = []
        self.qkv_out_scales_gpu = []
        
        self.out_linear_in_scales = []
        self.out_linear_out_scales = []
        self.out_linear_out_scales_gpu = []
        self.ffn1_in_scales = []
        self.ffn1_out_scales = []
        self.ffn1_out_scales_gpu = []

        self.ffn2_in_scales = []
        self.ffn2_out_scales = []
        self.ffn2_out_scales_gpu = []
    ...
    def GetBaselineOut(self):
        ...
        for i in range(self.layers):
            ...
            # print("x_data/dro+resid:", tensor_query[0][0][0])
            if self.pre_layer_norm:
                self.print_matrix(tensor_query.numpy(), "ln_in_{}.txt".format(i))
            ...
            self.qkv_out_scales_gpu.append(1.0/127.0)

            # print("step2: in_T/step9_out", ln1_out[0][0][0])
            print("qkv: inscale", max_v)
            self.print_matrix(ln1_out.numpy(), "qkv_in_{}.txt".format(i))
            
            # quant ln1_out
            ln1_out = self.fake_quant(ln1_out, self.qkv_in_scales[i])
            # print("step2: in_int8", ln1_out[0][0][0])
            ...
            self.print_matrix(out_linear_in.numpy(), "out_linear_in_{}.txt".format(i))

            max_v = paddle.max(
                paddle.abs(paddle.cast(out_linear_in, 'float32'))
            )[0]
            print("out_linear: inscale", max_v)

            self.out_linear_in_scales.append(1 / max_v)
            self.out_linear_out_scales.append(max_v / (127.0 * 127.0))
            self.out_linear_out_scales_gpu.append(1.0/127.0)
            ...
            self.print_matrix(out.numpy(), "out_linear_out_{}.txt".format(i))

            out = out + self.out_linear_proj_bias_tensor

            # print("step5:in ", out[0][0][0])
            # print("step5:resid ", residual[0][0][0])
            ...
            self.print_matrix(ffn1_out.numpy(), "ffn1_out_{}.txt".format(i))
            ...
            print("ffn2: inscale", max_v)
            self.ffn2_in_scales.append(1 / max_v)
            self.ffn2_out_scales.append(max_v / (127.0 * 127.0))
            self.ffn2_out_scales_gpu.append(1.0/127.0) # paddle.to_tensor([1.0/127.0]).astype('float32')
            self.print_matrix(ffn1_out.numpy(), "ffn2_in_{}.txt".format(i))
            ...
            self.print_matrix(ffn2_out.numpy(), "ffn2_out_{}.txt".format(i))
            ffn2_out = ffn2_out + self.ffn2_proj_bias_tensor

            # print("step9: in:", ffn2_out[0][0][0])
            # print("step9: residual:", attn_out[0][0][0])
            residual_out = attn_out + self.dropout(ffn2_out)
            
            final_out = residual_out
            if not self.pre_layer_norm:
                final_out = self.ffn_norm(residual_out)
            # print("step9: out:", final_out[0][0][0])
    ...
    def GetFusedMultiTransformerOut(self):
        ...
        for i in range(self.layers):
            ...
            qkv_out_scale = (
                ...
                * self.qkv_out_scales_gpu[i]
            )

            out_linear_out_scale = (
                paddle.ones([self.embed_dim], 'float32')
                * self.out_linear_out_scales_gpu[i]
            )

            ffn1_out_scale = (
                paddle.ones([4 * self.embed_dim], 'float32')
                * self.ffn1_out_scales_gpu[i]
            )

            ffn2_out_scale = (
                paddle.ones([self.embed_dim], 'float32')
                * self.ffn2_out_scales_gpu[i]
            )
        ...
        final_out = fused_multi_transformer_dyquant(
            x,
            ln_scales,
            ln_biases,
            qkv_weights,
        ...
        if self.has_cache_kv:
            ...
            if self.gen_cache_kv:
                ...
                for i in range(self.layers):
                    ...
                    if i == 0:
                        break
        # np.savetxt("../../final_out_ref.txt",final_out_ref[0].numpy())
        # np.savetxt("../../final_out.txt", final_out[0].numpy())
        np.testing.assert_allclose(
...
class TestFusedMultiTransformerDyquantOpFp16(TestFusedMultiTransformerDyquantOp):
    def config(self):
        super().config()
...
class TestFusedMultiTransformerDyquantOpCacheKV(TestFusedMultiTransformerDyquantOp):
    def config(self):
        super().config()
...
#     "FusedMultiTransformerDyquant requires CUDA >= 11.2 and CUDA_ARCH >= 8",
# )
# class TestFusedMultiTransformerDyquantOpCacheKVFp16(
#     TestFusedMultiTransformerDyquantOp
# ):
#     def config(self):
#         super().config()
#         self.has_cache_kv = True
...
class TestFusedMultiTransformerDyquantOpGenCacheKV(
    TestFusedMultiTransformerDyquantOp
):
    def config(self):
        super().config()
...
class TestFusedMultiTransformerDyquantOpGenCacheKVFp16(
    TestFusedMultiTransformerDyquantOp
):
    def config(self):
        super().config()
...
#     "FusedMultiTransformerDyquant requires CUDA >= 11.2 and CUDA_ARCH >= 8",
# )
# class TestFusedMultiTransformerDyquantOpPostLayerNormFp16(
#     TestFusedMultiTransformerDyquantOp
# ):
#     def config(self):
#         super().config()
...
#     "FusedMultiTransformerDyquant requires CUDA >= 11.2 and CUDA_ARCH >= 8",
# )
# class TestFusedMultiTransformerDyquantOpCacheKVPostLayerNorm(
#     TestFusedMultiTransformerDyquantOp
# ):
#     def config(self):
#         super().config()
...
#     "FusedMultiTransformerDyquant requires CUDA >= 11.2 and CUDA_ARCH >= 8",
# )
# class TestFusedMultiTransformerDyquantOpCacheKVPostLayerNormFp16(
#     TestFusedMultiTransformerDyquantOp
# ):
#     def config(self):
#         super().config()
...
#     "FusedMultiTransformerDyquant requires CUDA >= 11.2 and CUDA_ARCH >= 8",
# )
# class TestFusedMultiTransformerDyquantOpGenCacheKVPostLayerNorm(
#     TestFusedMultiTransformerDyquantOp
# ):
#     def config(self):
#         super().config()
...
#     "FusedMultiTransformerDyquant requires CUDA >= 11.2 and CUDA_ARCH >= 8",
# )
# class TestFusedMultiTransformerDyquantOpGenCacheKVPostLayerNormFp16(
#     TestFusedMultiTransformerDyquantOp
# ):
#     def config(self):
#         super().config()
