...
DeleteWeightDequantLinearOpEncoderPass::DeleteWeightDequantLinearOpEncoderPass() {
...
void DeleteWeightDequantLinearOpEncoderPass::ApplyImpl(ir::Graph* graph) const {
  ...
  PADDLE_ENFORCE_NOT_NULL(
      ...
      platform::errors::InvalidArgument(
          "Scope in DeleteWeightDequantLinearOpEncoderPass should not be null."));
  // Create pattern
    ...
    if (quant_axis == -1) {  // per_layer quant_dequant: all OP
      ...
      any_op2_desc->SetAttr("weight_scale", weight_scale[0]);
        ...
        PADDLE_THROW(platform::errors::Unimplemented(
            "Delete Weight Dequant Linear Op Encoder Pass is not supported for per-channel quantization"));
    }
  ...
  graph->Set("is_decoder", new bool(false));
