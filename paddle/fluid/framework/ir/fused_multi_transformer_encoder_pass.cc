...
PDNode* FusedMultiTransformerEncoderFuseQKVPattern::operator()(bool enable_int8) {
...
int FusedMultiTransformerEncoderFuseQKVPass::BuildFusion(
  ...
  auto fuse_creater = [&](Node* input0,
    ...
    // create fused_multi_transformer
    OpDesc fused_multi_transformer_op_desc(layer_norm->Op()->Block());
    fused_multi_transformer_op_desc.SetType(enable_int8 ? "fused_multi_transformer_int8" : "fused_multi_transformer");
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    VLOG(0) << "handle MultiTransformer encoder(Fuse-QKV) fuse";
    GET_IR_NODE_FROM_SUBGRAPH(
