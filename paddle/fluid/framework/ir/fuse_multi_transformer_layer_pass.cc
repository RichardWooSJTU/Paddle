...
std::unordered_map<std::string, std::string> MultiTransformerLayerPattern::operator()(
...
int FuseMultiTransformerLayerPass::BuildFusion(
    ...
    if (!enable_int8) {
        VLOG(4) << "fuse_multi_transformer_layer_pass will match float transformer op "
    ...
    patterns::MultiTransformerLayerPattern multi_layer_pattern(pattern, name_scope);
