...
inline void ChannelWiseScales(
  ...
  for (int i = 0; i < k; ++i) {
    for (int j = 0; j < n; ++j) {
      ...
      float v = abs(static_cast<float>(data[index]));
...
void QuantFusedMultiTrasformerPass::ApplyImpl(ir::Graph* graph) const {
  ...
  for (const Node* n : graph->Nodes()) {
    if (n->IsOp()) {
      ...
      op->SetType("fused_multi_transformer_dyquant");
      ...
      for (auto weight_var_name : op->Input("QKVW")) {
        ...
        if (scope.FindVar(weight_var_name + "_out_scale") == nullptr) {
          ...
          phi::Full<float>(*dev_ctx, {n_size}, 0, qkv_out_scale_tensor);
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        for (auto weight_var_name : weights_to_quant) {
          ...
          if (scope.FindVar(weight_var_name + "_out_scale") == nullptr) {
            ...
            phi::Full<float>(*dev_ctx, {n_size}, 0, out_scale_tensor);
