...
FuseMultiLayerTransformerPattern::operator()(bool enable_int8,
                                         int num_fused_op,
                                         bool is_decoder) {
...
int FuseMultiLayerTransformerPass::BuildFusion(Graph* graph,
  ...
  if (num_fuse_op == 0) {
    VLOG(4) << "fuse_multi_layer_transformer_pass will be skipped "
               "cause num_fuse_op is not been set or set to 0";
    return 0;
  }
  if (!is_decoder) {
    VLOG(4) << "fuse_multi_layer_transformer_pass will match encoder pattern";
  } else {
    VLOG(4) << "fuse_multi_layer_transformer_pass will match decoder pattern";
  }

  patterns::FuseMultiLayerTransformerPattern multi_layer_pattern(pattern,
                                                             name_scope);
...
void FuseMultiLayerTransformerPass::ApplyImpl(Graph* graph) const {
  ...
  PADDLE_ENFORCE_NOT_NULL(
      scope,
      platform::errors::Fatal("During the fuse_multi_layer_transformer pass, "
