...
template <typename T>
void fmha(cudaStream_t stream, const T *qkv_tensor, const T *qkv_bias_tensor,
          const T *src_mask_tensor, T *cache_k_tensor, T *cache_v_tensor,
          T *out_tensor, int batch_size, int max_seq_length, int num_head,
          int dim_head, const int64_t timestep, float inv_sqrt_dh) {
  Masked_multihead_attention_params<T> params;
...
template <typename T>
TransformerDecoderPluginDynamic<T>::TransformerDecoderPluginDynamic(
...
template <typename T>
size_t TransformerDecoderPluginDynamic<T>::getSerializationSize() const
    TRT_NOEXCEPT {
...
template <typename T>
void TransformerDecoderPluginDynamic<T>::serialize(void *buffer) const
    TRT_NOEXCEPT {
...
template <typename T>
int TransformerDecoderPluginDynamic<T>::enqueue(
  ...
  if (input_type == nvinfer1::DataType::kFLOAT) {
    VLOG(1) << "TRT Plugin DataType selected. "
               "TransformerDecoderPluginDynamic-->fp32";
    PADDLE_THROW(platform::errors::Fatal("unsupported float format!!!"));

  } else if (input_type == nvinfer1::DataType::kHALF) {
    VLOG(1) << "TRT Plugin DataType selected. "
               "TransformerDecoderPluginDynamic-->fp16";

    int64_t time_step = 0;
    const int64_t *d_time_step = static_cast<const int64_t *>(inputs[4]);
    // cudaMemcpy(&time_step, d_time_step, sizeof(int64_t),
    // cudaMemcpyDeviceToHost);
    // VLOG(3) << "bsz: " << bsz << "; max_seq_len: " << max_seq_len << ";
    // time_step: " << time_step;
    ...
    fmha<half>(stream, qkv_tensor, const_cast<const half *>(p_gpu_bias_),
               bias_qk, const_cast<half *>(k_cache),
               const_cast<half *>(v_cache), output, bsz, max_seq_len,
               head_number_, head_size_, 1, scale_);
  }
