...
template <typename T,
          ...
          int ELTS_PER_ROW_PER_CTA = THREADS_PER_ROW *VecSize,
          int LDGS = ELTS_PER_ROW / ELTS_PER_ROW_PER_CTA>
__global__ __launch_bounds__(THREADS_PER_CTA) void fused_fast_ln_fwd_quant_kernel(
    int rows,
    int cols,
    uint64_t seed,
    const float dropout_prob,
...
#pragma unroll
    ...
    Vec_int8 x_int8[LDGS]; // = static_cast<phi::AlignedVector<int8_t, VecSize>>(x);
...
#pragma unroll
    for (int it = 0, col = c; it < LDGS; it++) {
      // store x_int8[it] into y_ptr, 32 % vecsize == 0 to avoid the inner loop in VecSize.
      // 32 % vecsize == 0
      // for jt col loop: 
      phi::Store<int8_t, VecSize>(x_int8[it], y_ptr + row * ELTS_PER_ROW + col * VecSize);
      col += THREADS_PER_ROW;
    }
  }
}
...
template <typename T,
          ...
          int ELTS_PER_ROW_PER_CTA = THREADS_PER_ROW *VecSize,
          int LDGS = ELTS_PER_ROW / ELTS_PER_ROW_PER_CTA>
__global__ __launch_bounds__(THREADS_PER_CTA) void fused_fast_ln_fwd_quant_dequant_kernel(
    int rows,
    int cols,
    uint64_t seed,
    const float dropout_prob,
    ...
    const uint64_t increment,
    const float epsilon,
    const int32_t *__restrict__ x_ptr,
    const T *__restrict__ residual_ptr,
    const T *__restrict__ bias_ptr,
    const ScaleT *__restrict__ gamma_ptr,
    const ScaleT *__restrict__ beta_ptr,
...
template <typename T,
          ...
          int ELTS_PER_ROW_PER_CTA = THREADS_PER_ROW *VecSize,
          int LDGS = ELTS_PER_ROW / ELTS_PER_ROW_PER_CTA>
__global__ __launch_bounds__(THREADS_PER_CTA) void fused_fast_ln_fwd_dequant_kernel(
    int rows,
    int cols,
    uint64_t seed,
    const float dropout_prob,
...
template <typename T,
          ...
          typename U,
          bool ScaleBiasWithSameTypeX = false>
void LaunchLayernormResidualDropoutBiasQ(
    const uint32_t rows,
    const uint32_t cols,
    const int increment,
    uint64_t seed,
...
#define LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(cols)                           \
  case (cols): {                                                               \
    constexpr int WARPS_N = cols < 1024 ? 1 : (cols / 1024);                   \
    constexpr int WARPS_M = 4 / WARPS_N;                                       \
    const int THREADS_PER_WARP = 32;                                           \
    ...
    const int grid =                                                           \
        static_cast<int>(std::ceil(rows / static_cast<float>(ROWS_PER_CTA)));  \
    fused_fast_ln_fwd_quant_kernel<                                                  \
        T,                                                                     \
        U,                                                                     \
        LayerNormScaleBiasT<T, U, ScaleBiasWithSameTypeX>,                     \
        uint8_t,                                                               \
...
#define LAUNCH_FUSED_FAST_LN_KERNEL       \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(768);  \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(1024); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(1280); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(1536); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(1792); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(2048); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(3072); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_BASE(4096)
...
template <typename T,
          ...
          typename U,
          bool ScaleBiasWithSameTypeX = false>
void LaunchLayernormResidualDropoutBiasQDQ(
    const uint32_t rows,
    const uint32_t cols,
    const int increment,
    uint64_t seed,
...
#define LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_DEQUANT_BASE(cols)                           \
  case (cols): {                                                               \
    constexpr int WARPS_N = cols < 1024 ? 1 : (cols / 1024);                   \
    constexpr int WARPS_M = 4 / WARPS_N;                                       \
    const int THREADS_PER_WARP = 32;                                           \
    ...
    const int grid =                                                           \
        static_cast<int>(std::ceil(rows / static_cast<float>(ROWS_PER_CTA)));  \
    fused_fast_ln_fwd_quant_dequant_kernel<                                                  \
        T,                                                                     \
        U,                                                                     \
        LayerNormScaleBiasT<T, U, ScaleBiasWithSameTypeX>,                     \
        uint8_t,                                                               \
...
#define LAUNCH_FUSED_FAST_LN_KERNEL       \
  ...
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_DEQUANT_BASE(1536); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_DEQUANT_BASE(1792); \
  LAUNCH_FUSED_FAST_LN_KERNEL_QUANT_DEQUANT_BASE(2048); \
...
template <typename T,
          ...
          typename U,
          bool ScaleBiasWithSameTypeX = false>
void LaunchLayernormResidualDropoutBiasDQ(
    const uint32_t rows,
    const uint32_t cols,
    const int increment,
    uint64_t seed,
...
#define LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(cols)                           \
  case (cols): {                                                               \
    constexpr int WARPS_N = cols < 1024 ? 1 : (cols / 1024);                   \
    constexpr int WARPS_M = 4 / WARPS_N;                                       \
    const int THREADS_PER_WARP = 32;                                           \
    ...
    const int grid =                                                           \
        static_cast<int>(std::ceil(rows / static_cast<float>(ROWS_PER_CTA)));  \
    fused_fast_ln_fwd_dequant_kernel<                                                  \
        T,                                                                     \
        U,                                                                     \
        LayerNormScaleBiasT<T, U, ScaleBiasWithSameTypeX>,                     \
        uint8_t,                                                               \
...
#define LAUNCH_FUSED_FAST_LN_KERNEL       \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(768);  \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(1024); \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(1280); \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(1536); \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(1792); \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(2048); \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(3072); \
  LAUNCH_FUSED_FAST_LN_KERNEL_DEQUANT_BASE(4096)
