...
class FusedMultiTransformerINT8OpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    auto place = ctx.GetPlace();
    // PADDLE_ENFORCE_EQ(place==platform::CUDAPlace(), true, 
    //   platform::errors::PreconditionNotMet("FusedMultiTransformerINT8OpKernel must be in cuda place"));
    ...
    //debuggg
    VLOG(1) << "ln_scales dtype " << ln_scales[0]->dtype();
    if (std::is_same<T, float>::value) {
      VLOG(1) << "run with float";
    } else {
      VLOG(1) << "run with fp16";
    }
    //debuggg end
...
cudaDeviceSynchronize();
    ...
    gettimeofday(&start, NULL);
      ...
      if (time_step) {  // generation decoder stage
        ...
        VLOG(1) << "fmha generation decoder stage";
        ...
        VLOG(1) << "fmha generation context stage";
