                                                 ...
                                                 const float* scale_ptr)
{
    ...
    if (check) {
        char4 tmp4;
        tmp4.x = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id)) * scale);
        tmp4.y = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id + 1)) * scale);
        tmp4.z = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id + 2)) * scale);
        tmp4.w = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id + 3)) * scale);
        // COL32_col = n_id >> 5 ; COL32_row = (m_id << 5) + (n_id & 31)
        // COL32_idx = (COL32_col << 5) * m + COL32_row = (n_id & 0xffffffe0)*m + (m_id << 5) + (n_id & 31)
        output[((n_id & 0xffffffe0) * m + (m_id << 5) + (n_id & 31)) >> 2] = tmp4;
    }
}
...
void row_major_to_col32_quantize_kernelLauncher(const T* input,
                                                ...
                                                cudaStream_t stream) {
  dim3 grid(m);
  dim3 block(min(1024, n));

  row_major_to_col32_quantize_kernel<T><<<grid, block, 0, stream>>>(
      input,
      (char4*)output,
      m,
      n,
...
template <typename T>
__global__ void col32_to_row_major_dequant_kernel(T* output,
                                                  const int32_t* input,
                                                  const T* scale,
                                                  const T* w_scale,
                                                  const int m,  // hidden
                                                  ...
                                                  const float max_range) 
{
  ...
  if (check) {
    output[n_id * m + m_id] =
        (T)((float)(input[(m_id & 0xffffffe0) * n + (n_id << 5) + (m_id & 31)]) *
            (float)scale[n_id] / max_range * (float)w_scale[m_id] / max_range);
  }
}
...
void col32_to_row_major_dequantize_kernelLauncher(const int32_t* input,
  ...
  col32_to_row_major_dequant_kernel<<<grid, block, 0, stream>>>(
...
public:
    ...
    ~AttnMatmulINT8(){}
    void ComputeForward(const framework::Tensor* weight, // [fp16/32] 
                        framework::Tensor* weight_tmp, // [int8] workspace
        ...
        // row_major_to_col32_quantize_kernelLauncher<T>(input->data<T>(), input_tmp->data<int8_t>(), &scale, bsz_seq_, input_size_, dev_ctx_.stream());

        VLOG(1) << "[DEBUG] GEMM";
        VLOG(1) << "input_tmp " << input_tmp->numel();
        VLOG(1) << "weight_tmp " << weight_tmp->numel();
        VLOG(1) << "output_tmp " << output_tmp->numel();

        helper_ -> GEMM(input_tmp->data<int8_t>(), weight_tmp->data<int8_t>(), output_tmp->data<int32_t>(), dev_ctx_.stream());

        //dequant C
        VLOG(1) << "[DEBUG] col32_to_row_major_dequantize_kernelLauncher";
        // col32_to_row_major_dequantize_kernelLauncher<T>(output_tmp->data<int32_t>(), &scale, &scale, output->data<T>(), bsz_seq_, output_size_, dev_ctx_.stream());
