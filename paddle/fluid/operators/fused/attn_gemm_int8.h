                                                 ...
                                                 int n)
{
    ...
    if (check) {
        char4 tmp4;
        tmp4.x = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id)) * 1.0);
        tmp4.y = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id + 1)) * 1.0);
        tmp4.z = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id + 2)) * 1.0);
        tmp4.w = float_to_int8_rn(static_cast<float>(__ldg(input + m_id * n + n_id + 3)) * 1.0);
        // COL32_col = n_id >> 5 ; COL32_row = (m_id << 5) + (n_id & 31)
        // COL32_idx = (COL32_col << 5) * m + COL32_row = (n_id & 0xffffffe0)*m + (m_id << 5) + (n_id & 31)
        output[((n_id & 0xffffffe0) * m + (m_id << 5) + (n_id & 31)) >> 2] = tmp4;
    }
                                                  ...
                                                  const float max_range) 
{
  ...
  if (check) {
    output[n_id * m + m_id] =
        ((float)(input[(m_id & 0xffffffe0) * n + (n_id << 5) + (m_id & 31)]) *
